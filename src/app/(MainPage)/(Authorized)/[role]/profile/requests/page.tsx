        ...
        {requests.map((r) => {
          ...
          return (
            <NextLink
              key={r.id}
              href={`${pathname}/${r.id}`}
              className="block rounded-2xl border border-[#E4E0D8] bg-white p-5 transition-all hover:shadow-md hover:border-[#B3541E]"
            >
              ...
              <div className="mt-1 text-sm text-[#5B554F]">
                {r.town}, {r.district}, {r.state}
              </div>

              <div className="mt-4 flex items-center justify-between border-t border-[#E4E0D8] pt-3 text-sm">
                <span className="text-[#5B554F]">
                  Needed by {formatDate(r.neededAt)}
                </span>

                <span className="font-medium text-[#1B1714]">
                  {donatedUnits}/{r.unitsNeeded} units
                </span>
              </div>
            </NextLink>
          );
